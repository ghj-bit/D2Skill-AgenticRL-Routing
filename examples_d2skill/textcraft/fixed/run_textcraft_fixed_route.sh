...
export TEXTCRAFT_HISTORY_LENGTH="${TEXTCRAFT_HISTORY_LENGTH:-0}"
...
export TEXTCRAFT_AGENT_PROMPT_STYLE="${TEXTCRAFT_AGENT_PROMPT_STYLE:-agentgym}"
export TEXTCRAFT_KEEP_FULL_ASSISTANT_HISTORY_INCLUDE_THINK="${TEXTCRAFT_KEEP_FULL_ASSISTANT_HISTORY_INCLUDE_THINK:-1}"
...
export TEXTCRAFT_FIXED_SKILL_IDS="${TEXTCRAFT_FIXED_SKILL_IDS:-}"
export TEXTCRAFT_FIXED_SKILLS_TASK_IDS="${TEXTCRAFT_FIXED_SKILLS_TASK_IDS:-}"
RUNS="${RUNS:-3}"
...
MODEL_LOG_DIR="${MODEL_LOG_DIR:-${LOG_ROOT}/${FIXED_ROUTE_MODEL}}"
...
for ((run_idx = 0; run_idx < RUNS; run_idx++)); do
    ...
    bash "${EXAMPLES_DIR}/run_textcraft_d2skill_gigpo.sh" "$ENGINE" \
        ...
        +env.textcraft_agent_prompt_style="$TEXTCRAFT_AGENT_PROMPT_STYLE" \
